...
export type WorkflowStep = 'select_case' | 'select_modality';
...
export interface DocketOption {
  ...
  unit: string;
  statutory: string;
  status: 'PRIMARY ACTIVE' | 'ACTIVE INVESTIGATION' | 'SURVEILLANCE';
  isPrimary?: boolean;
}

export const DOCKET_OPTIONS: DocketOption[] = [
  {
    id: 'case-382',
    caseNumber: 'CASE #382/2026',
    name: 'Operation Syndicate Viper',
    firReference: 'FIR #382/2026 PS Lanka (Primary Active)',
    target: 'Vikramaditya @ Vicky Kashi',
    unit: 'Varanasi Range',
    statutory: 'Sec 111 BNS',
    status: 'PRIMARY ACTIVE',
    isPrimary: true,
  },
  {
    id: 'case-405',
    caseNumber: 'CASE #405/2026',
    name: 'Purvanchal Hawala Transit',
    firReference: 'FIR #114/2026 PS Cantt',
    target: 'Tariq @ Al-Nahda Handler',
    unit: 'STF Special Cell Cantt',
    statutory: 'Sec 318(4) BNS // PMLA',
    status: 'ACTIVE INVESTIGATION',
  },
  {
    id: 'case-291',
    caseNumber: 'CASE #291/2026',
    name: 'Cyber Churn & SIM Farm',
    firReference: 'Cyber Crime Unit VNS',
    target: 'Burner SIM Farm Facilitator',
    unit: 'Cyber Crime Unit VNS',
    statutory: 'Sec 66 IT Act // Sec 4 Telegraph Act',
    status: 'SURVEILLANCE',
  },
];
...
export const MultiFormatIngestModal: React.FC<MultiFormatIngestModalProps> = ({
  ...
  const [workflowStep, setWorkflowStep] = useState<WorkflowStep>('select_case');
  const [selectedDocket, setSelectedDocket] = useState<DocketOption>(DOCKET_OPTIONS[0]);
  ...
  useEffect(() => {
    if (isOpen) {
      setWorkflowStep('select_case');
      setSelectedDocket(DOCKET_OPTIONS[0]);
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/85 backdrop-blur-xs p-4 sm:p-6 font-sans">
      <div className="relative w-full max-w-2xl bg-slate-900 border border-slate-700 rounded-[3px] shadow-2xl overflow-hidden my-auto text-slate-100 flex flex-col max-h-[90vh]">
        {/* Modal Header */}
        <div className="p-4 bg-slate-950 border-b border-slate-800 flex items-center justify-between shrink-0">
          <div className="flex items-center space-x-3">
            <div className="w-8 h-8 rounded-[2px] bg-slate-900 border border-slate-700 flex items-center justify-center text-blue-400 shrink-0">
            ...
            <div>
              ...
              <div className="text-[11px] font-mono text-slate-400">
                {workflowStep === 'select_case'
                  ? 'Step 1: Select Active Docket // Mandated Under BSA Section 63 Protocol'
                  : `Target Docket: ${selectedDocket.caseNumber} (${selectedDocket.name}) // Sec 63 BSA Chain of Custody`}
              </div>
            </div>
          </div>
          ...
          <button
            ...
            title="Close Ingestion Suite (Esc)"
          >
            <X className="w-4 h-4" />
          </button>
        </div>

        {/* 1. Modal Header State Tracker: Breadcrumbs */}
        <div className="px-6 py-2.5 bg-slate-950/90 border-b border-slate-800 flex items-center justify-between text-xs font-mono shrink-0 select-none">
          <div className="flex items-center space-x-2.5">
            <button
              type="button"
              onClick={() => {
                if (workflowStep === 'select_modality' && !isProcessing) {
                  setWorkflowStep('select_case');
                }
              }}
              className={`flex items-center space-x-1.5 transition-colors ${
                workflowStep === 'select_case'
                  ...
                  : 'text-slate-400 hover:text-cyan-300 cursor-pointer'
              }`}
            >
              <span className={`w-4 h-4 rounded-full flex items-center justify-center text-[10px] font-bold ${
                workflowStep === 'select_case'
                  ? 'bg-cyan-500/20 border border-cyan-500/60 text-cyan-300 shadow-[0_0_8px_rgba(6,182,212,0.4)]'
                  : 'bg-emerald-950 border border-emerald-700 text-emerald-400'
              }`}>
                {workflowStep === 'select_modality' ? '✓' : '1'}
              </span>
              <span>Step 1: Select Case Docket</span>
            </button>

            <span className="text-slate-600 font-bold">➔</span>

            <div className={`flex items-center space-x-1.5 ${
              workflowStep === 'select_modality'
                ? 'text-cyan-400 font-bold'
                : 'text-slate-500'
            }`}>
              <span className={`w-4 h-4 rounded-full flex items-center justify-center text-[10px] font-bold ${
              ...
              <span>Step 2: Evidence Modality &amp; Upload</span>
            </div>
          </div>

          {/* Step 2 Back Link */}
          {workflowStep === 'select_modality' && !isProcessing && !isCompleted && (
            <button
              type="button"
              onClick={() => setWorkflowStep('select_case')}
              className="text-[11px] font-mono text-slate-400 hover:text-cyan-300 flex items-center gap-1 transition-colors px-2 py-0.5 rounded-[2px] border border-slate-700 hover:border-cyan-700 bg-slate-900"
            >
        ...
        <div className="p-6 space-y-6 overflow-y-auto flex-1">
          {/* ========================================================================= */}
          {/* STEP 1: SELECT TARGET CASE / DOCKET (Default View on Open)                */}
          {/* ========================================================================= */}
          {workflowStep === 'select_case' && (
            <div className="space-y-4 animate-in fade-in duration-150">
              <div className="text-xs text-slate-300 leading-relaxed font-sans">
                Evidence records must be cryptographically bound to an active FIR or investigation docket prior to ingestion:
              </div>

              {/* Active Dockets Card Selector */}
              <div className="space-y-2.5">
                {DOCKET_OPTIONS.map((docket) => {
                  const isSelected = selectedDocket.id === docket.id;
                  return (
                    <div
                      key={docket.id}
                      onClick={() => setSelectedDocket(docket)}
                      className={`p-3.5 rounded-[2px] border cursor-pointer transition-all flex items-center justify-between ${
                        isSelected
                    ...
                    >
                      <div className="space-y-1 min-w-0 flex-1 pr-3">
                        ...
                        <div className="text-[11px] font-mono text-slate-400 flex items-center space-x-2">
                          <span>Target: <span className="text-slate-200">{docket.target}</span></span>
                          <span className="text-slate-600">•</span>
                          <span>Unit: <span className="text-slate-200">{docket.unit}</span></span>
                        </div>
                      </div>

                      <div className="flex items-center space-x-3 shrink-0">
                        <span className={`text-[9.5px] font-mono px-2 py-0.5 rounded-[2px] border font-bold ${
                          docket.status === 'PRIMARY ACTIVE'
                            ? 'bg-rose-950 text-rose-300 border-rose-800'
                            : docket.status === 'ACTIVE INVESTIGATION'
                ...
                {/* Option for: + Create New Investigation Docket */}
                <button
                  type="button"
                  onClick={() => {
                    onClose();
                    if (onOpenRegisterCase) {
                      onOpenRegisterCase();
                    }
                  }}
                  className="w-full p-3 bg-slate-950/40 hover:bg-slate-900 border border-dashed border-slate-700 hover:border-cyan-500/50 rounded-[2px] flex items-center justify-center space-x-2 text-xs font-mono text-cyan-400 transition-colors group cursor-pointer"
                >
                  <Plus className="w-3.5 h-3.5 group-hover:scale-110 transition-transform" />
                  <span className="font-bold">+ Create New Investigation Docket</span>
                </button>
              </div>

              {/* Compact Summary Pill of Selected Case */}
              <div className="p-3 bg-slate-950 border border-slate-800 rounded-[2px] flex items-center justify-between text-xs font-mono">
                <div className="flex flex-wrap items-center gap-2 text-slate-300 truncate">
                  <span className="text-cyan-400 font-bold">Target:</span>
                  <span className="text-white font-semibold">{selectedDocket.target}</span>
                  <span className="text-slate-600">|</span>
                  <span className="text-slate-400">STF Unit:</span>
                  <span className="text-slate-200">{selectedDocket.unit}</span>
                  <span className="text-slate-600">|</span>
                  <span className="text-slate-400">Statutory:</span>
                  <span className="text-amber-400">{selectedDocket.statutory}</span>
                </div>
                <span className="text-[10px] font-mono px-2 py-0.5 bg-emerald-950 text-emerald-300 border border-emerald-800 rounded font-bold shrink-0">
                  BSA SEC 63 READY
                </span>
              </div>

              {/* Primary Action Button: Confirm Case & Proceed to Ingestion */}
              <button
                type="button"
                onClick={() => setWorkflowStep('select_modality')}
                className="w-full py-2.5 px-4 bg-blue-700 hover:bg-blue-600 text-white font-mono text-xs font-bold uppercase tracking-wider rounded-[2px] transition-colors border border-blue-600 flex items-center justify-center space-x-2 shadow-md group cursor-pointer"
              >
                <span>Confirm Case &amp; Proceed to Ingestion</span>
          ...
          {/* ========================================================================= */}
          {/* STEP 2: EVIDENCE MODALITY SELECTION & UPLOAD (Current Screen)             */}
          {/* ========================================================================= */}
          {workflowStep === 'select_modality' && (
            <div className="space-y-6 animate-in fade-in duration-150">
              {!isProcessing && !isCompleted && (
                <div className="space-y-4">
                  <div className="flex items-center justify-between text-xs text-slate-300 leading-relaxed font-sans">
                    <span>Select an evidence modality to initiate AI-powered entity extraction, bilingual OCR, and cellular tower cross-referencing:</span>
                  </div>
                    ...
                    {INGEST_FORMATS.map((fmt) => {
                      ...
                      return (
                        <button
                          ...
                          className="p-3.5 bg-slate-950 hover:bg-slate-850 border border-slate-800 hover:border-slate-600 rounded-[2px] text-left transition-colors group flex items-start space-x-3 shadow-sm cursor-pointer"
                        >
                  ...
                  >
                    ...
                    <div className="text-[11px] text-slate-500 font-sans">
                      Supports PDF, CSV, WAV/MP3, MP4, JSON (Max 500MB per batch) • Bound to [{selectedDocket.caseNumber}]
                    </div>
                  </div>
                </div>
              )}
                ...
                <div className="py-4 space-y-4">
                  <div className="flex items-center justify-between">
                    <div className="flex items-center space-x-2 min-w-0">
                      ...
                      <span className="text-xs font-mono font-bold text-white truncate">
                        INGESTING TO [{selectedDocket.caseNumber}]: {uploadedFiles.length > 0 ? uploadedFiles[0].name : selectedFormat.sampleFile}
                      </span>
                    </div>
              ...
              {isCompleted && (
                <div className="space-y-5 py-2">
                  <div className="p-4 bg-emerald-950/40 border border-emerald-700/60 rounded-[2px] flex items-start space-x-3">
                    ...
                    <div className="text-xs space-y-1">
                      ...
                      <p className="text-slate-300 leading-relaxed text-[11.5px]">
                        {uploadedFiles.length > 0
                          ? `${uploadedFiles.length} evidence records (${uploadedFiles.map(f => f.name).join(', ')}) successfully sealed and linked to docket ${selectedDocket.caseNumber} (${selectedDocket.name}). Integrated into the central Purvanchal Syndicate graph.`
                          : `Intelligence successfully sealed and bound to docket ${selectedDocket.caseNumber} (${selectedDocket.name}). All extracted entities and relationships are cryptographically bound.`}
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="pt-3 border-t border-slate-800 flex items-center justify-between">
                    <button
                      onClick={handleReset}
                      className="px-3 py-1.5 bg-slate-800 hover:bg-slate-700 text-slate-300 rounded-[2px] text-xs font-mono transition-colors cursor-pointer"
                    >
                    ...
                    <button
                      ...
                      className="px-4 py-2 bg-blue-700 hover:bg-blue-600 text-white font-mono text-xs font-bold uppercase tracking-wider rounded-[2px] flex items-center space-x-1.5 transition-colors border border-blue-600 cursor-pointer shadow-md"
                    >
